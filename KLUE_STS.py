...
class KoNET:
    def __init__(self, firstTraining, use_attention_supervision=True):
        ...
        self.input_ids_arr = np.load('sts_data/input_ids.npy')
        self.token_type_ids_arr = np.load('sts_data/token_type_ids.npy')
        self.label_arr = np.load('sts_data/label.npy')
        self.attention_supervision_arr = np.load('sts_data/pos_attention.npy')
        ...
        self.save_path = './sts_data/model/sts_model_plus.ckpt'
        # self.bert_path = '/home/ai/pycharm_project/koelectra/koelectra_base_v3'
        self.bert_path = '/data/pretrain_models/klue_bert/klue_bert.ckpt'
    ...
    def Training(self, is_Continue, training_epoch):
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(training_epoch):
                ...
                loss_, loss2_, _ = sess.run([total_loss, loss, optimizer], feed_dict=feed_dict)

                if attention_supervision_loss is not None:
                    attention_supervision_loss_value = sess.run(attention_supervision_loss, feed_dict=feed_dict)
                    print(i, '/', training_epoch, loss_, loss2_, 'attention_loss:', attention_supervision_loss_value)
    ...
    def eval_nli_with_file(self, epoch):
        ...
        all_input_ids = np.load('klue_nli_dev/input_ids.npy')
        all_token_type_ids = np.load('klue_nli_dev/token_type_ids.npy')
        all_label_ids = np.load('klue_nli_dev/label.npy')
        ...
        with tf.Session(config=config) as sess:
            ...
            for ep in range(28):
                ...
                for j in range(100):
                    #print(prediction_idx[j], label_idx[j])
    ...
    def eval_nli(self):
        data = json.load(open('/data/KLUE-baseline/data/klue_benchmark/klue-sts-v1.1/klue-sts-v1.1_dev.json', 'r', encoding='utf-8'))
        label_dict = {0: 0, 1: 1}
